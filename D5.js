...
const invertedOrder = orderedPets.reverse();
console.log(invertedOrder);
...
const moveToLast = pets.push(FirstPet);
console.log(pets);
...
cars.push({
  ...
  model: "For-Two",
  color: "white",
  trims: ["pure", "passion", "prime"],
});

for (i = 0; i < cars.length; i++) {
  const car = cars[i];
  const lastItem = car.trims.shift();
